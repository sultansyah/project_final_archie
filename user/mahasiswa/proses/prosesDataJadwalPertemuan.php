...
function tambah_data()
{
    ...
    if (empty($judul)) {
        ...
        $tambah = mysqli_query($conn, "INSERT INTO tb_jadwal_pertemuan(judul, keterangan, mahasiswa, kategori, tanggal_jam, id_user_pembuat, level_pembuat)  VALUES ('$judul','$keterangan','$nim','$kategori',  " . ($tanggal_jam == NULL ? "NULL" : "'$tanggal_jam'") . ",'$id_user_pembuat', '$level_pembuat')");
...
function edit_data()
{
    ...
    $update = mysqli_query($conn, "UPDATE tb_jadwal_pertemuan SET judul='$judul',keterangan='$keterangan', mahasiswa='$nim',kategori='$kategori',tanggal_jam='$tanggal_jam',id_user_pengedit='$id_user_pengedit',level_pengedit='$level_pengedit' WHERE id_jadwal = '$id_jadwal'");
...
function link_tujuan()
{
    ...
    if ($_POST['tanggal_jam'] < date('d-m-Y H:i:s')) {
        redirect_page("Tolong edit jam pertemuan anda dengan jam yang sekarang", "jp");
    } else {
